...
Target: `kotlin/bc-shamir/` package `com.blockchaincommons.bcshamir`
...
| Rust dependency | Purpose | Kotlin/JVM equivalent |
|---|---|---|
| `thiserror` | Error derive macro | Kotlin exception class hierarchy (no macro needed) |
| `hex-literal` (dev) | Hex test vectors | Local hex parser helper in tests |
| `hex` (dev) | Hex formatting in tests/debug | Not required for final Kotlin tests |
| `version-sync` (dev) | Rust metadata checks | Not applicable to Kotlin package; omit as Rust-specific |
| `rand` (dev) | `RngCore`/`CryptoRng` impl in tests | Not required; use Kotlin fake RNG implementing bc-rand API |

Internal BC deps for Kotlin implementation:
- `bc-rand` -> `com.blockchaincommons:bcrand` API via composite build (`includeBuild("../bc-rand")`)
- `bc-crypto` -> `com.blockchaincommons:bccrypto` API via composite build (`includeBuild("../bc-crypto")`)
...
Kotlin translation test requirement:
...
1. `TU-1` constants and error model
   ...
   - `Error` -> Kotlin exception class with matching cases
...
1. **GF(2^8) bit-slicing semantics**
   ...
   - Kotlin must use `UInt` arrays and explicit shifts to preserve semantics.
...
4. **Generic `AsRef<[u8]>` inputs**
   - Rust accepts flexible share containers; Kotlin should accept `List<ByteArray>` for ergonomic parity while preserving behavior.

5. **Memory zeroization expectations**
   - Rust zeroes temporary buffers with `memzero` utilities.
   - Kotlin should call `memzero`/`memzeroAll` from `bc-crypto` for parity.

6. **Deterministic fake RNG vectors**
   - Test vectors depend on a fake RNG that fills bytes with `0, 17, 34, ...` wrapping at 256.
   - Kotlin test fake RNG must replicate this exactly.
