        ...
        do {
            ...
            case CBOR.tagged(let tag, let cbor):
                ...
                case KnownValue.cborTag:
                    ...
                    let knownValue = KnownValuesStore.knownValue(
                        forRawValue: rawValue,
                        in: context?.knownValues
                    )
                    return knownValue.description.flanked("'")
                ...
                case URI.cborTag:
                    return try URI(untaggedCBOR: cbor).description.flanked("URI(", ")")
                case UUID.cborTag:
                    return try UUID(untaggedCBOR: cbor).description.lowercased().flanked("UUID(", ")")
                case Digest.cborTag:
