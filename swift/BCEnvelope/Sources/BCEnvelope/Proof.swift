...
public extension Envelope {
    ...
    func proof(contains target: Swift.Set<Digest>) -> Envelope? {
    ...
    func confirm(contains target: Swift.Set<Digest>, proof: Envelope) -> Bool {
...
extension Envelope {
    func revealSet(of target: Swift.Set<Digest>) -> Swift.Set<Digest> {
        var result: Swift.Set<Digest> = []
        revealSets(of: target, current: [], result: &result)
        return result
    }

    func containsAll(in target: Swift.Set<Digest>) -> Bool {
        var target = target
        removeAllFound(in: &target)
        return target.isEmpty
    }

    func revealSets(of target: Swift.Set<Digest>, current: Swift.Set<Digest>, result: inout Swift.Set<Digest>) {
        var current = current
    ...
    func removeAllFound(in target: inout Swift.Set<Digest>) {
        if target.contains(digest) {
