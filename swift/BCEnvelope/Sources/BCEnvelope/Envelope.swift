...
public extension Envelope {
    ...
    init(_ subject: Any) {
        ...
        } else if let encryptedMessage = subject as? EncryptedMessage, encryptedMessage.hasDigest {
            try! self.init(encryptedMessage: encryptedMessage)
        } else if let compressed = subject as? Compressed, compressed.hasDigest {
...
extension Envelope {
    init(subject: Envelope, uncheckedAssertions: [Envelope]) {
        ...
        let digest = Digest.fromImage(Data(digests.map { $0.data }.joined()))
    ...
    init(encryptedMessage: EncryptedMessage) throws {
        guard encryptedMessage.hasDigest else {
    ...
    init(compressed: Compressed) throws {
        guard compressed.hasDigest else {
            throw EnvelopeError.missingDigest
        }
    ...
    init(leaf: CBOR) {
        let digest = Digest.fromImage(leaf.cborData)
        self = .leaf(leaf, digest)
    }

    init(wrapped envelope: Envelope) {
        let digest = Digest.fromImage(envelope.digest.data)
