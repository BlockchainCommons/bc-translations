...
public extension Envelope {
    ...
    func addSalt(_ count: Int) throws -> Envelope {
        guard let salt = try? Salt.newWithLen(count) else {
    ...
    func addSalt(_ range: ClosedRange<Int>) throws -> Envelope {
        guard let salt = try? Salt.newInRange(range) else {
            throw EnvelopeError.invalidFormat
        }
        return addSalt(salt)
    }
    ...
    func addSalt<R: BCRandomNumberGenerator>(using rng: inout R) -> Envelope {
        addSalt(Salt.newForSizeUsing(taggedCBOR.cborData.count, rng: &rng))
    }
}
