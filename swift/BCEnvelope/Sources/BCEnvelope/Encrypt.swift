...
public extension Envelope {
    ...
    func encryptSubject(with key: SymmetricKey, testNonce: Nonce? = nil) throws -> Envelope {
        ...
        case .node(let subject, let assertions, let envelopeDigest):
            ...
            let encryptedMessage = key.encryptWithDigest(encodedCBOR, digest: subjectDigest, nonce: testNonce)
        ...
        case .leaf(let cbor, let envelopeDigest):
            ...
            let encryptedMessage = key.encryptWithDigest(encodedCBOR, digest: envelopeDigest, nonce: testNonce)
        ...
        case .wrapped(_, let wrappedDigest):
            ...
            let encryptedMessage = key.encryptWithDigest(encodedCBOR, digest: wrappedDigest, nonce: testNonce)
            result = try Envelope(encryptedMessage: encryptedMessage)
        ...
        case .assertion(let assertion):
            ...
            let encryptedMessage = key.encryptWithDigest(encodedCBOR, digest: assertionDigest, nonce: testNonce)
            result = try Envelope(encryptedMessage: encryptedMessage)
        ...
        case .compressed(let compressed):
            ...
            let encryptedMessage = key.encryptWithDigest(encodedCBOR, digest: compressedDigest, nonce: testNonce)
            result = try Envelope(encryptedMessage: encryptedMessage)
    ...
    func decryptSubject(with key: SymmetricKey) throws -> Envelope {
        ...
        let encodedCBOR = try key.decrypt(message)

        guard message.hasDigest else {
            throw EnvelopeError.missingDigest
        }
        let subjectDigest = message.digest
