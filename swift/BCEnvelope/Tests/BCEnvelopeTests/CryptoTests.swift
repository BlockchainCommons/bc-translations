...
//        print(envelope.ur)
        ...
        #expect(throws: (any Swift.Error).self) { try receivedEnvelope.verifySignature(from: carolPublicKeys) }
        ...
        #expect(throws: (any Swift.Error).self) { try receivedEnvelope.verifySignatures(from: [alicePublicKeys, carolPublicKeys], threshold: 2) }
    }
...
//        print(envelope.ur)
        ...
        #expect(throws: (any Swift.Error).self) { try receivedEnvelope.extractSubject(String.self) }

        // Can't read with incorrect key.
        #expect(throws: (any Swift.Error).self) { try receivedEnvelope.decryptSubject(with: SymmetricKey()) }
    }
...
//        print(envelope.ur)
        ...
        #expect(throws: (any Swift.Error).self) { try receivedEnvelope.decrypt(to: alicePrivateKeys) }
    }
...
//        print(recoveredSeedEnvelope.format())
        ...
        #expect(throws: (any Swift.Error).self) { try Envelope(shares: [bobEnvelope]) }
    }
}
