...
public struct QRLogo: Sendable {
    ...
    private static func renderSVGViaCoreGraphics(_ data: Data, size: CGSize, context: CGContext) -> Bool {
        ...
        typealias CreateFunc = @convention(c) (CFData) -> Unmanaged<AnyObject>?
        ...
        guard let documentRef = create(data as CFData) else { return false }
        let document = documentRef.takeRetainedValue()

        let svgSize = getSize(document)
        guard svgSize.width > 0, svgSize.height > 0 else { return false }
