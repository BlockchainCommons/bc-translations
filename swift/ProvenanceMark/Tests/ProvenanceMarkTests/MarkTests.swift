...
@MainActor
struct MarkTests {
    /// Generates 10 provenance marks and validates them against expected test vectors.
    ///
    /// This helper mirrors the Rust `run_test` function in `tests/mark.rs`.
    private func runTest(
        ...
        let dates: [Date] = (0..<count).map { i in
        ...
        var marks: [ProvenanceMark] = []

        // Generate marks using the JSON serialize/deserialize pattern from Rust.
        var encodedGenerator = try JSONEncoder().encode(provenanceGen)
        for date in dates {
            var generator = try JSONDecoder().decode(
                ProvenanceMarkGenerator.self, from: encodedGenerator
            )
            // NOTE: Cannot use a ternary here because CBOR conforms to
            // ExpressibleByNilLiteral, so `condition ? CBOR("...") : nil`
            // produces CBOR.null (.some(.null)) instead of Optional.none.
            let title: CBOR?
            if includeInfo {
                title = CBOR("Lorem ipsum sit dolor amet.")
            } else {
                title = nil
            }
            let result = generator.next(date: date, info: title)
            encodedGenerator = try JSONEncoder().encode(generator)
            marks.append(result)
        }

        // Validate the sequence.
        #expect(ProvenanceMark.isSequenceValid(marks))
        ...
        if !expectedDebug.isEmpty {
            for (i, mark) in marks.enumerated() {
                #expect(
                    mark.debugDescription == expectedDebug[i],
                    "debug mismatch at index \(i)"
                )
            }
        }

        // Check bytewords encoding.
        if !expectedBytewords.isEmpty {
            for (i, mark) in marks.enumerated() {
                #expect(
                    mark.bytewords() == expectedBytewords[i],
                    "bytewords mismatch at index \(i)"
                )
            }
        }

        // Check identifier words.
        ...
        // Check UR strings.
        if !expectedURs.isEmpty {
            for (i, mark) in marks.enumerated() {
                #expect(
                    mark.urString() == expectedURs[i],
                    "UR mismatch at index \(i)"
                )
            }
        }

        // Check URLs.
        let baseURL = "https://example.com/validate"
        if !expectedURLs.isEmpty {
            for (i, mark) in marks.enumerated() {
                #expect(
                    mark.url(base: baseURL).absoluteString == expectedURLs[i],
        ...
        // Bytewords roundtrip.
        for (i, mark) in marks.enumerated() {
            let bw = mark.bytewords()
            let decoded = try ProvenanceMark(
                resolution: resolution, bytewords: bw
            )
            #expect(decoded == mark, "bytewords roundtrip failed at index \(i)")
        }

        // UR roundtrip.
        for (i, mark) in marks.enumerated() {
            let urStr = mark.urString()
            let decoded = try ProvenanceMark.fromURString(urStr)
            #expect(decoded == mark, "UR roundtrip failed at index \(i)")
        }

        // URL roundtrip.
        for (i, mark) in marks.enumerated() {
            let url = mark.url(base: baseURL)
            let decoded = try ProvenanceMark(url: url)
            #expect(decoded == mark, "URL roundtrip failed at index \(i)")
        }

        // JSON roundtrip for each mark.
        let encoder = JSONEncoder()
        let decoder = JSONDecoder()
        for (i, mark) in marks.enumerated() {
            let jsonData = try encoder.encode(mark)
            let decoded = try decoder.decode(ProvenanceMark.self, from: jsonData)
            #expect(decoded == mark, "JSON roundtrip failed at index \(i)")
        }
    }

    // MARK: - Test: Low resolution, no info
    ...
    @Test func testLow() throws {
        ...
        let expectedIdWords = [
            "HELP UNDO WASP LAZY", "FUEL KNOB FERN VISA", "FILM HILL PART LION", "FLAP SILK DATA OBEY",
            "LAVA MILD POSE QUIZ", "FAIR SONG SURF LUNG", "FLAP FUND ITEM WHIZ", "HOPE OMIT HILL PART",
            "VIAL IRON DICE MANY", "KITE VOWS YOGA ROOF",
        ]
        let expectedBytemojiIds = [
            "\u{1F32E} \u{1F430} \u{1F984} \u{1F494}",
            "\u{1F34D} \u{1FA90} \u{1F9B6} \u{1F435}",
            "\u{1F34A} \u{1F371} \u{1F9EE} \u{1F6A9}",
            "\u{1F349} \u{1F525} \u{1F47D} \u{1FA91}",
            "\u{1F49B} \u{1F680} \u{1F4E1} \u{1F381}",
            "\u{1F91A} \u{1FA73} \u{1F454} \u{1F6D1}",
            "\u{1F349} \u{1F95D} \u{1F344} \u{1F422}",
            "\u{1F364} \u{1F48C} \u{1F371} \u{1F9EE}",
            "\u{1F42E} \u{1F341} \u{1F63B} \u{1F697}",
            "\u{1F4AB} \u{1F425} \u{1FABC} \u{1F3C6}",
        ]
        let expectedURs = [
        ...
        try runTest(
            ...
            expectedDebug: expectedDebug,
            expectedBytewords: expectedBytewords,
            expectedIdWords: expectedIdWords,
            expectedBytemojiIds: expectedBytemojiIds,
            expectedURs: expectedURs,
            expectedURLs: expectedURLs
        )
    }

    // MARK: - Test: Low resolution, with info
    ...
    @Test func testLowWithInfo() throws {
        ...
        let expectedIdWords = [
            "ROAD WAXY EDGE SAGA", "KING LAVA LEGS KITE", "GUSH MANY WASP ZERO", "WOLF EVEN GURU CRUX",
            "RUBY HUTS UNDO FLAP", "VOID TIME YURT IRON", "GALA CURL RAMP WALL", "WOLF TAXI IRON ZINC",
            "ROOF GEMS ONYX JUDO", "HOLY KICK LUCK NAVY",
        ]
        let expectedBytemojiIds = [
            "\u{1F52D} \u{1F41B} \u{1F4AA} \u{1F3BE}",
            "\u{1F30E} \u{1F49B} \u{1F496} \u{1F4AB}",
            "\u{1F9C0} \u{1F697} \u{1F984} \u{1F41A}",
            "\u{1F43A} \u{1F9B7} \u{1F35E} \u{1F608}",
            "\u{1F941} \u{1F960} \u{1F430} \u{1F349}",
            "\u{1F414} \u{1F45F} \u{1F99E} \u{1F341}",
            "\u{1F346} \u{1F479} \u{1F389} \u{1F986}",
            "\u{1F43A} \u{1F9F5} \u{1F341} \u{1F41F}",
            "\u{1F3C6} \u{1F345} \u{1F4E6} \u{1F4A8}",
            "\u{1F35C} \u{1F31C} \u{1F7E9} \u{1F3A1}",
        ]
        let expectedURs = [
    ...
    // MARK: - Test: Medium resolution, no info
    ...
    @Test func testMedium() throws {
        ...
        let expectedIdWords = [
            "CATS LUNG JADE TUNA", "OPEN VOID NAVY BARN", "WHAT HARD AQUA MONK", "CALM CATS CRUX WEBS",
            "PLAY BULB SWAN FIZZ", "RUIN GRIM UNIT CRUX", "BLUE NUMB TASK VAST", "OVAL EXAM LUAU BARN",
            "NOON JURY BALD WORK", "OBOE MONK UNDO KITE",
        ]
        let expectedBytemojiIds = [
            "\u{1F920} \u{1F6D1} \u{1F339} \u{1F436}",
            "\u{1F4EB} \u{1F414} \u{1F3A1} \u{1F929}",
            "\u{1F40C} \u{1F354} \u{1F644} \u{1F6A6}",
            "\u{1F922} \u{1F920} \u{1F608} \u{1F98B}",
            "\u{1F48E} \u{1F635} \u{1F9E2} \u{1F34C}",
            "\u{1F3B7} \u{1F96F} \u{1F98A} \u{1F608}",
            "\u{1F975} \u{1F6AA} \u{1F9F6} \u{1F428}",
            "\u{1F4D6} \u{1F442} \u{1F537} \u{1F929}",
            "\u{1F514} \u{1F300} \u{1F971} \u{1F40D}",
            "\u{1F388} \u{1F6A6} \u{1F430} \u{1F4AB}",
        ]
        let expectedURs = [
    ...
    // MARK: - Test: Medium resolution, with info
    ...
    @Test func testMediumWithInfo() throws {
        ...
        let expectedIdWords = [
            "NAIL NEED ITEM EASY", "TACO ZINC WAND SURF", "OBEY STUB AXIS LIMP", "YOGA YANK AUNT LEGS",
            "EACH KICK HUTS SETS", "INTO NEXT TACO HOPE", "TACO BELT IRON FLEW", "ZINC BETA FLEW PART",
            "JADE EDGE GRIM TINY", "LIST WHEN SETS LEAF",
        ]
        let expectedBytemojiIds = [
            "\u{1F3F0} \u{1F3A2} \u{1F344} \u{1F448}",
            "\u{1F453} \u{1F41F} \u{1F989} \u{1F454}",
            "\u{1FA91} \u{1F457} \u{1F62D} \u{1F3C1}",
            "\u{1FABC} \u{1FABD} \u{1F60D} \u{1F496}",
            "\u{1F44E} \u{1F31C} \u{1F960} \u{2728}",
            "\u{1F331} \u{1F3E0} \u{1F453} \u{1F364}",
            "\u{1F453} \u{1F636} \u{1F341} \u{1F347}",
            "\u{1F41F} \u{1F928} \u{1F347} \u{1F9EE}",
            "\u{1F339} \u{1F4AA} \u{1F96F} \u{1F9E6}",
            "\u{1F4AC} \u{1F41E} \u{2728} \u{1F498}",
        ]
        let expectedURs = [
    ...
    // MARK: - Test: Quartile resolution, no info
    ...
    @Test func testQuartile() throws {
        ...
        let expectedIdWords = [
            "GAME AUNT EXIT OMIT", "TUNA HILL SCAR HANG", "ONYX DELI WOLF TIED", "JUDO JOWL POEM NUMB",
            "DOOR TOYS LION LAVA", "MEOW CALM NOTE CURL", "FUEL NEWS PLUS HORN", "HANG HIGH RUBY TENT",
            "TASK SILK DAYS RUNS", "OMIT AWAY TIME WHIZ",
        ]
        let expectedBytemojiIds = [
            "\u{1F951} \u{1F60D} \u{1F443} \u{1F48C}",
            "\u{1F436} \u{1F371} \u{1F3D3} \u{1F32D}",
            "\u{1F4E6} \u{1F639} \u{1F43A} \u{1F460}",
            "\u{1F4A8} \u{1F338} \u{23F0} \u{1F6AA}",
            "\u{1F640} \u{1F45C} \u{1F6A9} \u{1F49B}",
            "\u{1F6A8} \u{1F922} \u{1F511} \u{1F479}",
            "\u{1F34D} \u{1F3A0} \u{1F4F7} \u{1F35A}",
            "\u{1F32D} \u{1F959} \u{1F941} \u{1F48D}",
            "\u{1F9F6} \u{1F525} \u{1F63A} \u{1F3BA}",
            "\u{1F48C} \u{1F618} \u{1F45F} \u{1F422}",
        ]
        let expectedURs = [
    ...
    // MARK: - Test: Quartile resolution, with info
    ...
    @Test func testQuartileWithInfo() throws {
        ...
        let expectedIdWords = [
            "HELP ROOF SURF PLAY", "NEED COST LAMB VISA", "LOUD DELI DULL QUIZ", "EVEN FERN CURL SAGA",
            "LOVE MONK DICE PAID", "GOOD POOL WARM EXAM", "LIAR VETO ICED JUMP", "DATA SOAP LIMP BIAS",
            "NAVY URGE NEXT PECK", "CODE BALD JOWL HOLY",
        ]
        let expectedBytemojiIds = [
            "\u{1F32E} \u{1F3C6} \u{1F454} \u{1F48E}",
            "\u{1F3A2} \u{1F92F} \u{1F310} \u{1F435}",
            "\u{1F6AB} \u{1F639} \u{1F91D} \u{1F381}",
            "\u{1F9B7} \u{1F9B6} \u{1F479} \u{1F3BE}",
            "\u{1F534} \u{1F6A6} \u{1F63B} \u{1F4CC}",
            "\u{1F9C4} \u{23F3} \u{1F434} \u{1F442}",
            "\u{1F495} \u{1F981} \u{1F368} \u{1F4A7}",
            "\u{1F47D} \u{1F45A} \u{1F3C1} \u{1FAE5}",
            "\u{1F3A1} \u{1F43B} \u{1F3E0} \u{1F512}",
            "\u{1F62C} \u{1F971} \u{1F338} \u{1F35C}",
        ]
        let expectedURs = [
    ...
    // MARK: - Test: High resolution, no info
    ...
    @Test func testHigh() throws {
        ...
        let expectedIdWords = [
            "LUCK TRIP VIEW COST", "SILK RUST FLUX URGE", "CURL COST GYRO JUDO", "ZERO INTO YOGA CITY",
            "ECHO TASK FIZZ DIET", "MINT ZONE FUND GAME", "NAIL EPIC JOWL UNIT", "PART BRAG CATS WHIZ",
            "OWLS NOTE PECK FISH", "JUGS GOOD AWAY BODY",
        ]
        let expectedBytemojiIds = [
            "\u{1F7E9} \u{1F431} \u{1F438} \u{1F92F}",
            "\u{1F525} \u{1F3C0} \u{1F353} \u{1F43B}",
            "\u{1F479} \u{1F92F} \u{1F95A} \u{1F4A8}",
            "\u{1F41A} \u{1F331} \u{1FABC} \u{1F973}",
            "\u{1F446} \u{1F9F6} \u{1F34C} \u{1F63D}",
            "\u{1F681} \u{1F42C} \u{1F95D} \u{1F951}",
            "\u{1F3F0} \u{1F444} \u{1F338} \u{1F98A}",
            "\u{1F9EE} \u{1F633} \u{1F920} \u{1F422}",
            "\u{1F4DA} \u{1F511} \u{1F512} \u{1F34B}",
            "\u{1F30A} \u{1F9C4} \u{1F618} \u{1F976}",
        ]
        let expectedURs = [
    ...
    // MARK: - Test: High resolution, with info
    ...
    @Test func testHighWithInfo() throws {
        ...
        let expectedIdWords = [
            "BARN JADE BARN CLAW", "WHEN JUMP CUSP DOOR", "JOIN OPEN IRON RICH", "BLUE VISA SAFE DULL",
            "JOLT FROG FLAP USER", "TUNA NEWS EASY COLA", "CITY WOLF BLUE MONK", "BODY CHEF AQUA PLUS",
            "GEMS ZEST HALF GEMS", "COOK HELP TIME HORN",
        ]
        let expectedBytemojiIds = [
            "\u{1F929} \u{1F339} \u{1F929} \u{1F97A}",
            "\u{1F41E} \u{1F4A7} \u{1F47A} \u{1F640}",
            "\u{1F33C} \u{1F4EB} \u{1F341} \u{1FAD6}",
            "\u{1F975} \u{1F435} \u{1F3C8} \u{1F91D}",
            "\u{1F33B} \u{1F351} \u{1F349} \u{1F43C}",
            "\u{1F436} \u{1F3A0} \u{1F448} \u{1F911}",
            "\u{1F973} \u{1F43A} \u{1F975} \u{1F6A6}",
            "\u{1F976} \u{1F921} \u{1F644} \u{1F4F7}",
            "\u{1F345} \u{1F9AD} \u{1F357} \u{1F345}",
            "\u{1F643} \u{1F32E} \u{1F45F} \u{1F35A}",
        ]
        let expectedURs = [
