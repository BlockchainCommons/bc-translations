...
func runSchnorrVector(t *testing.T, vector schnorrVector) {
	t.Helper()

	if vector.secretKey != "" && vector.auxRand != "" {
		secretKey := must32(vector.secretKey)
		auxRand := must32(vector.auxRand)
		actualPublicKey := SchnorrPublicKeyFromPrivateKey(secretKey)
		if actualPublicKey != must32(vector.publicKey) {
			t.Fatalf("public key mismatch\n got  %x\n want %x", actualPublicKey, must32(vector.publicKey))
		}

		actualSignature := SchnorrSignWithAuxRand(secretKey, mustHex(vector.message), auxRand)
		if actualSignature != must64(vector.signature) {
			t.Fatalf("signature mismatch\n got  %x\n want %x", actualSignature, must64(vector.signature))
		}
	}

	verified := SchnorrVerify(must32(vector.publicKey), must64(vector.signature), mustHex(vector.message))
	if verified != vector.verifies {
		t.Fatalf("verify mismatch\n got  %v\n want %v", verified, vector.verifies)
	}
}
...
func TestSchnorrSign(t *testing.T) {
	...
	sig := SchnorrSignUsing(privateKey, message, rng)
	if sig != must64("8f6ec4edbe1a6d96edfc5f15e18e06a6e2559a3426c52d2c38fec17fe7e0cafc95177206d018662a279f2b571224cf07006939fc25d0cae7a7e7b44a4b25f543") {
		t.Fatalf("signature = %x", sig)
	}

	schnorrPublicKey := SchnorrPublicKeyFromPrivateKey(privateKey)
	if !SchnorrVerify(schnorrPublicKey, sig, message) {
		t.Fatalf("SchnorrVerify returned false")
	}
}

func TestBIP340Vectors(t *testing.T) {
	vectors := []schnorrVector{
		{
	...
	for i, vector := range vectors {
		...
		t.Run(fmt.Sprintf("vector_%d", i), func(t *testing.T) {
			runSchnorrVector(t, vector)
		})
	}
}

func TestInvalidPublicKeyVectorsPanic(t *testing.T) {
	vectors := []schnorrVector{
		{
	...
	for _, vector := range vectors {
		func() {
			defer func() {
				if r := recover(); r == nil {
					t.Fatalf("expected panic for invalid x-only public key")
				}
			}()
			runSchnorrVector(t, vector)
		}()
...
func TestVerifyTweaked(t *testing.T) {
	message := []byte("message")
	publicKey := must32("b1ca6327b48b3f2f11c80b460aeff6934cbf1705083792108be9545b53818472")
	signature := must64("cddfdf12ffa1698b2fa7449bd6aa4581cdab05205864cddaba1a137a1db132ea2f4255a81199c58241087036f5b66ec4303409cd7d760039729f78f19db004dc")
