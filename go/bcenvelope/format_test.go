...
func TestPlaintext(t *testing.T) {
	...
	assertActualExpected(t, envelope.FormatFlat(), `"Hello."`)

	assertActualExpected(t, envelope.TreeFormat(),
		`8cc96cdb "Hello."`)

	assertActualExpected(t,
		envelope.TreeFormatOpt(TreeFormatOpts{
			HighlightingTarget: make(map[bccomponents.Digest]bool),
			Context:            FormatOptNone(),
		}),
		`8cc96cdb "Hello."`)
	...
	assertActualExpected(t,
		envelope.TreeFormatOpt(TreeFormatOpts{
			...
			DigestDisplay:      DigestDisplayFull,
		}),
	...
	treeLines := strings.Split(envelope.TreeFormat(), "\n")
	if envelope.ElementsCount() != len(treeLines) {
		t.Errorf("elements count %d != tree format line count %d", envelope.ElementsCount(), len(treeLines))
	}
}

func TestSignedPlaintextFormat(t *testing.T) {
	rng := bcrand.NewFakeRandomNumberGenerator()
	options := &bccomponents.SigningOptions{SchnorrRNG: rng}
	envelope := NewEnvelope(plaintextHello).AddSignatureOpt(
		alicePrivateKey().SchnorrSigningPrivateKey(),
		options,
		nil,
	)
	...
	assertActualExpected(t, envelope.FormatFlat(),
		`"Hello." [ 'signed': Signature ]`)

	s := envelope.TreeFormat()
	assertActualExpected(t, s,
		`949a991e NODE
    8cc96cdb subj "Hello."
    fcb4e2be ASSERTION
	...
	s = envelope.TreeFormatOpt(TreeFormatOpts{
		HighlightingTarget: make(map[bccomponents.Digest]bool),
		Context:            FormatOptNone(),
	})
	assertActualExpected(t, s,
...
func TestEncryptSubjectFormat(t *testing.T) {
	envelope, err := NewEnvelope("Alice").
		AddAssertion("knows", "Bob").
		EncryptSubject(bccomponents.NewSymmetricKey())
	if err != nil {
	...
	actual := envelope.MermaidFormat()
	expected := `%%{ init: { 'theme': 'default', 'flowchart': { 'curve': 'basis' } } }%%
graph LR
0(("NODE<br>8955db5e"))
...
func TestTopLevelAssertion(t *testing.T) {
	envelope := NewAssertionEnvelope("knows", "Bob")

	assertActualExpected(t, envelope.Format(), `"knows": "Bob"`)

	assertActualExpected(t, envelope.FormatFlat(), `"knows": "Bob"`)
...
func TestElidedObject(t *testing.T) {
	envelope := NewEnvelope("Alice").AddAssertion("knows", "Bob")
	elided := envelope.ElideRemovingTarget(NewEnvelope("Bob"))

	assertActualExpected(t, elided.Format(),
		`"Alice" [
    "knows": ELIDED
]`)

	assertActualExpected(t, elided.FormatFlat(),
		`"Alice" [ "knows": ELIDED ]`)

	assertActualExpected(t, elided.TreeFormat(),
		`8955db5e NODE
    13941b48 subj "Alice"
    78d666eb ASSERTION
        db7dd21c pred "knows"
	...
	assertActualExpected(t,
		elided.TreeFormatOpt(TreeFormatOpts{
			HideNodes:          true,
			HighlightingTarget: make(map[bccomponents.Digest]bool),
			Context:            FormatOptGlobal(),
		}),
	...
	treeLines := strings.Split(elided.TreeFormat(), "\n")
	if elided.ElementsCount() != len(treeLines) {
		t.Errorf("elements count %d != tree format line count %d", elided.ElementsCount(), len(treeLines))
	}
}

func TestSignedSubject(t *testing.T) {
	rng := bcrand.NewFakeRandomNumberGenerator()
	options := &bccomponents.SigningOptions{SchnorrRNG: rng}
	...
	target[envelope.Digest()] = struct{}{}
	target[envelope.Subject().Digest()] = struct{}{}
	elided := envelope.ElideRevealingSet(target)
	...
	assertActualExpected(t, elided.FormatFlat(), `"Alice" [ ELIDED (3) ]`)
...
func TestWrapThenSigned(t *testing.T) {
	rng := bcrand.NewFakeRandomNumberGenerator()
	options := &bccomponents.SigningOptions{SchnorrRNG: rng}
	envelope := NewEnvelope("Alice").
		AddAssertion("knows", "Bob").
...
func TestEncryptToRecipients(t *testing.T) {
	nonce := fakeNonce()
	envelope, err := NewEnvelope(plaintextHello).
		EncryptSubjectWithNonce(fakeContentKey(), &nonce)
	if err != nil {
		t.Fatalf("EncryptSubjectWithNonce failed: %v", err)
	}
	envelope = checkEncoding(t, envelope)
	envelope = envelope.AddRecipientOpt(
		bobPublicKey().EncapsulationPublicKey(),
		fakeContentKey(),
		&nonce,
	)
	envelope = checkEncoding(t, envelope)
	envelope = envelope.AddRecipientOpt(
		carolPublicKey().EncapsulationPublicKey(),
		fakeContentKey(),
		&nonce,
	)
	envelope = checkEncoding(t, envelope)
...
func TestAssertionPositions(t *testing.T) {
...
func TestComplexMetadata(t *testing.T) {
	...
	arid1Data := mustDecodeHex("9c747ace78a4c826392510dd6285551e7df4e5164729a1b36198e56e017666c8")
	var arid1Arr [bccomponents.ARIDSize]byte
	copy(arid1Arr[:], arid1Data)
	arid1 := bccomponents.ARIDFromData(arid1Arr)

	author := checkEncoding(t, NewEnvelope(arid1).
		AddAssertion(knownvalues.DereferenceVia, "LibraryOfCongress").
		AddAssertion(knownvalues.Name, "Ayn Rand"))

	nameEn := NewEnvelope("Atlas Shrugged").
		AddAssertion(knownvalues.Language, "en")

	nameEs := NewEnvelope("La rebelión de Atlas").
		AddAssertion(knownvalues.Language, "es")

	arid2Data := mustDecodeHex("7fb90a9d96c07f39f75ea6acf392d79f241fac4ec0be2120f7c82489711e3e80")
	var arid2Arr [bccomponents.ARIDSize]byte
	copy(arid2Arr[:], arid2Data)
	arid2 := bccomponents.ARIDFromData(arid2Arr)

	work := checkEncoding(t, NewEnvelope(arid2).
		AddAssertion(knownvalues.IsA, "novel").
		AddAssertion("isbn", "9780451191144").
		AddAssertion("author", author).
		AddAssertion(knownvalues.DereferenceVia, "LibraryOfCongress").
		AddAssertion(knownvalues.Name, nameEn).
		AddAssertion(knownvalues.Name, nameEs))

	bookData := "This is the entire book \u201cAtlas Shrugged\u201d in EPUB format."
	bookMetadata := checkEncoding(t, NewEnvelope(bccomponents.DigestFromImage([]byte(bookData))).
		AddAssertion("work", work).
		AddAssertion("format", "EPUB").
		AddAssertion(knownvalues.DereferenceVia, "IPFS"))

	assertActualExpected(t, bookMetadata.Format(),
		`Digest(26d05af5) [
    "format": "EPUB"
    "work": ARID(7fb90a9d) [
        'isA': "novel"
	...
	assertActualExpected(t, bookMetadata.FormatFlat(),
	...
	assertActualExpected(t, bookMetadata.TreeFormat(),
		`c93370e7 NODE
	...
	assertActualExpected(t,
		bookMetadata.TreeFormatOpt(TreeFormatOpts{
			HideNodes:          true,
			HighlightingTarget: make(map[bccomponents.Digest]bool),
			Context:            FormatOptGlobal(),
		}),
	...
	treeLines := strings.Split(bookMetadata.TreeFormat(), "\n")
	if bookMetadata.ElementsCount() != len(treeLines) {
		t.Errorf("elements count %d != tree format line count %d", bookMetadata.ElementsCount(), len(treeLines))
	}
}
...
func TestCredentialFormat(t *testing.T) {
	...
	assertActualExpected(t, cred.Format(),
		`{
    ARID(4676635a) [
	...
	assertActualExpected(t, cred.FormatFlat(),
	...
	assertActualExpected(t,
		cred.TreeFormatOpt(TreeFormatOpts{
			HideNodes:          true,
			HighlightingTarget: make(map[bccomponents.Digest]bool),
			Context:            FormatOptGlobal(),
		}),
	...
	treeLines := strings.Split(cred.TreeFormat(), "\n")
	if cred.ElementsCount() != len(treeLines) {
		t.Errorf("elements count %d != tree format line count %d", cred.ElementsCount(), len(treeLines))
	}
}

func TestRedactedCredential(t *testing.T) {
	redacted := redactedCredential()
	rng := bcrand.NewFakeRandomNumberGenerator()
	options := &bccomponents.SigningOptions{SchnorrRNG: rng}
	issueDate, _ := dcbor.DateFromString("2022-01-01")
	warranty := redacted.
		Wrap().
		AddAssertion("employeeHiredDate", issueDate).
		AddAssertion("employeeStatus", "active").
		Wrap().
		AddAssertion(knownvalues.Note, "Signed by Employer Corp.").
		AddSignatureOpt(bobPrivateKey().SchnorrSigningPrivateKey(), options, nil)
	warranty = checkEncoding(t, warranty)

	assertActualExpected(t, warranty.Format(),
		`{
    {
        {
            ARID(4676635a) [
	...
	assertActualExpected(t, warranty.FormatFlat(),
	...
	s = warranty.TreeFormatOpt(TreeFormatOpts{
		HideNodes:          true,
		HighlightingTarget: make(map[bccomponents.Digest]bool),
		Context:            FormatOptGlobal(),
	})
	...
	treeLines := strings.Split(warranty.TreeFormat(), "\n")
	if warranty.ElementsCount() != len(treeLines) {
		t.Errorf("elements count %d != tree format line count %d", warranty.ElementsCount(), len(treeLines))
	}

	actual := warranty.MermaidFormatOpt(MermaidFormatOpts{
		Theme:              MermaidThemeDark,
		HighlightingTarget: make(map[bccomponents.Digest]bool),
		Context:            FormatOptGlobal(),
	})
	expected := `%%{ init: { 'theme': 'dark', 'flowchart': { 'curve': 'basis' } } }%%
graph LR
0(("NODE<br>7ab3e6b1"))
...
linkStyle 49 stroke:magenta,stroke-width:2px`
	...
	actual = warranty.MermaidFormatOpt(MermaidFormatOpts{
		Monochrome:         true,
		Theme:              MermaidThemeForest,
		Orientation:        MermaidTopToBottom,
		HideNodes:          true,
		HighlightingTarget: make(map[bccomponents.Digest]bool),
		Context:            FormatOptGlobal(),
	})
	expected = `%%{ init: { 'theme': 'forest', 'flowchart': { 'curve': 'basis' } } }%%
graph TB
0[/"WRAPPED"\]
    0 -- subj --> 1[/"WRAPPED"\]
        1 -- subj --> 2[/"WRAPPED"\]
