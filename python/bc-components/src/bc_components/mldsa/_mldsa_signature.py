"""ML-DSA digital signature."""

from __future__ import annotations

from bc_tags import (
    CBOR,
    Tag,
    tags_for_values,
    TAG_MLDSA_SIGNATURE,
)

from .._error import InvalidSizeError
from ._mldsa_level import MLDSALevel


class MLDSASignature:
    """A digital signature created with the ML-DSA post-quantum algorithm.

    Stores a security level and raw signature bytes.
    """

    __slots__ = ("_level", "_data")

    def __init__(self, level: MLDSALevel, data: bytes) -> None:
        expected = level.signature_size()
        if len(data) != expected:
            raise InvalidSizeError("MLDSA signature", expected, len(data))
        self._level = level
        self._data = bytes(data)

    # --- Construction ---

    @staticmethod
    def from_bytes(level: MLDSALevel, data: bytes) -> MLDSASignature:
        """Create a signature from raw bytes and a security level."""
        return MLDSASignature(level, data)

    # --- Properties ---

    @property
    def level(self) -> MLDSALevel:
        """The security level of this signature."""
        return self._level

    @property
    def size(self) -> int:
        """The size of this signature in bytes."""
        return self._level.signature_size()

    @property
    def data(self) -> bytes:
        """The raw signature bytes."""
        return self._data

    # --- CBOR ---

    @staticmethod
    def cbor_tags() -> list[Tag]:
        return tags_for_values([TAG_MLDSA_SIGNATURE])

    def untagged_cbor(self) -> CBOR:
        return CBOR.from_array([
            self._level.to_cbor(),
            CBOR.from_bytes(self._data),
        ])

    def tagged_cbor(self) -> CBOR:
        tags = self.cbor_tags()
        return CBOR.from_tagged_value(tags[0], self.untagged_cbor())

    def tagged_cbor_data(self) -> bytes:
        return self.tagged_cbor().to_cbor_data()

    @staticmethod
    def from_untagged_cbor(cbor: CBOR) -> MLDSASignature:
        elements = cbor.try_array()
        if len(elements) != 2:
            raise ValueError("MLDSASignature must have two elements")
        level = MLDSALevel.from_cbor(elements[0])
        data = elements[1].try_byte_string()
        return MLDSASignature(level, data)

    @staticmethod
    def from_tagged_cbor(cbor: CBOR) -> MLDSASignature:
        tags = MLDSASignature.cbor_tags()
        item = cbor.try_expected_tagged_value(tags[0])
        return MLDSASignature.from_untagged_cbor(item)

    def to_cbor(self) -> CBOR:
        return self.tagged_cbor()

    def to_cbor_data(self) -> bytes:
        return self.tagged_cbor_data()

    # --- Dunder methods ---

    def __eq__(self, other: object) -> bool:
        if isinstance(other, MLDSASignature):
            return self._level == other._level and self._data == other._data
        return NotImplemented

    def __hash__(self) -> int:
        return hash((self._level, self._data))

    def __repr__(self) -> str:
        return f"{self._level.name}Signature"

    def __str__(self) -> str:
        return f"{self._level.name}Signature"
