...
class KnownValuesStore:
    ...
    @classmethod
    def new(cls, known_values: Iterable[KnownValue]) -> KnownValuesStore:
        """Create a new store from an iterable of KnownValue instances."""
    ...
    def assigned_name(self, known_value: KnownValue) -> str | None:
    ...
    def name(self, known_value: KnownValue) -> str:
        """Return the store-assigned name or the KnownValue default name."""
        assigned_name = self.assigned_name(known_value)
    ...
    def known_value_for_raw_value(
        ...
        if known_values is not None:
            ...
            if match is not None:
                return match
        return KnownValue.new(raw_value)
    ...
    def name_for_known_value(
        ...
        if known_values is not None:
            assigned_name = known_values.assigned_name(known_value)
            if assigned_name is not None:
                return assigned_name
        return known_value.name()
