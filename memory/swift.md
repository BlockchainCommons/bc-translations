...
- When multiple Swift test suites call shared registry setup in async initializers, gate setup with a `@MainActor` idempotent function; otherwise concurrent mutation of global stores can crash nondeterministically.
- `KnownValuesStore.shared` is an immutable value snapshot; for translation compatibility aliases (for example `signed` -> `verifiedBy`), build a mutable copy and inject it through local format context rather than trying to mutate `shared`.
