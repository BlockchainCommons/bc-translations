...
import { CborDate, cbor } from '@bc/dcbor';
import {
  ProvenanceMark,
...
function runTest(
  ...
  let generator = ProvenanceMarkGenerator.createWithPassphrase(resolution, 'Wolf');
  const count = 10;
  const baseTime = Date.UTC(2023, 5, 20, 12, 0, 0);
  const dates: CborDate[] = [];
  for (let i = 0; i < count; i++) {
    dates.push(CborDate.fromDatetime(new Date(baseTime + i * 86400000)));
  }

  const info = includeInfo ? cbor('Lorem ipsum sit dolor amet.') : undefined;
  ...
  // debug
  for (let i = 0; i < marks.length; i++) {
    expect(marks[i]!.toDebugString()).toBe(expectedDebug[i]);
  }

  // bytewords
  const bytewords = marks.map((m) => m.toBytewords());
  for (let i = 0; i < marks.length; i++) {
    expect(bytewords[i]).toBe(expectedBytewords[i]);
  }
  // round-trip bytewords
  for (let i = 0; i < marks.length; i++) {
    const rt = ProvenanceMark.fromBytewords(resolution, bytewords[i]!);
    expect(rt.equals(marks[i]!)).toBe(true);
  }
  ...
  // URs
  const urs = marks.map((m) => m.toUrString());
  for (let i = 0; i < marks.length; i++) {
    expect(urs[i]).toBe(expectedUrs[i]);
  }
  // round-trip URs
  for (let i = 0; i < marks.length; i++) {
    const rt = ProvenanceMark.fromUrString(urs[i]!);
    expect(rt.equals(marks[i]!)).toBe(true);
  }

  // URLs
  const urls = marks.map((m) => m.toUrl('https://example.com/validate'));
  for (let i = 0; i < marks.length; i++) {
    expect(urls[i]!.toString()).toBe(expectedUrls[i]);
  }
  // round-trip URLs
  for (let i = 0; i < marks.length; i++) {
    const rt = ProvenanceMark.fromUrl(urls[i]!);
    expect(rt.equals(marks[i]!)).toBe(true);
  }
  ...
  for (const mark of marks) {
    ...
    const rt = ProvenanceMark.fromJSON(json);
    expect(rt.equals(mark)).toBe(true);
  }
}

describe('mark', () => {
  test('test_readme_deps', async () => {
    ...
    expect(packageJson.dependencies['@bc/rand']).toBe('file:../bc-rand');
    expect(packageJson.dependencies['@bc/envelope']).toBe('file:../bc-envelope');
  });

  test('test_html_root_url', async () => {
    const source = await readFile(
      new URL('../src/index.ts', import.meta.url),
      'utf8',
    );

    expect(source.includes('@module @bc/provenance-mark')).toBe(true);
  });

  test('test_envelope', () => {
    ProvenanceMark.registerTags();

    const seed = ProvenanceSeed.createWithPassphrase('test');
    const date = CborDate.fromString('2025-10-26');

    const generator = ProvenanceMarkGenerator.createWithSeed(
      ProvenanceMarkResolution.High,
      seed,
    );
    const mark = generator.next(date, cbor('Info field content'));

    const generatorEnvelope = generator.toEnvelope();
    expect(generatorEnvelope.format()).toBe(`h'b16a7cbd178ee0d41cadb0dcefdbe87d6a41c85b41c551134ae8307f9203babc' [
    'isA': "provenance-generator"
    "next-seq": 1
    "res": 3
    "rng-state": h'f49e6d96b4988a0a3535e9e96d73041fdd1d697e03db6130904db44e9073bd4a'
    "seed": h'578aa064bafda09ccd91c44698ae25372e142dfbcf3be252bc3d70140c46d25a'
]`);

    const decodedGenerator = ProvenanceMarkGenerator.fromEnvelope(
      generatorEnvelope,
    );
