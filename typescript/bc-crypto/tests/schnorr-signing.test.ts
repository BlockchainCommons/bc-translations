...
function runTestVector(vector: TestVector): void {
    if (vector.secretKey && vector.auxRand) {
        const actualPublicKey = schnorrPublicKeyFromPrivateKey(
            hexToBytes(vector.secretKey),
        );
        expect(actualPublicKey).toEqual(hexToBytes(vector.publicKey));

        const actualSignature = schnorrSignWithAuxRand(
            hexToBytes(vector.secretKey),
            hexToBytes(vector.message),
            hexToBytes(vector.auxRand),
        );
        expect(actualSignature).toEqual(hexToBytes(vector.signature));
    }

    const verified = schnorrVerify(
        hexToBytes(vector.publicKey),
        hexToBytes(vector.signature),
        hexToBytes(vector.message),
    );
    expect(verified).toBe(vector.verifies);
}
